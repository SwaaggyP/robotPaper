...
class Config:
    ...
    def __init__(self):
        ...
        self.v_min = -0.5  # [m/s]
        # 角速度边界
        self.w_max = 90.0 * math.pi / 180.0  # [rad/s]
        self.w_min = -90.0 * math.pi / 180.0  # [rad/s]
        # 线加速度和角加速度最大值
        ...
        # 障碍物位置 [x(m) y(m), ....]   换成机器人的位置 
        self.ob = np.array([[10, 10]])
        # 目标点位置
        self.target = np.array([0, 15])
...
class DWA:
    ...
    def __dist(self, trajectory, obstacle):
        ...
        r = np.hypot(dx, dy)
        return np.min(r) if np.array(r < self.radius + 1).any() else self.judge_distance
...
def plot_robot(x, y, yaw, config):  # pragma: no cover
    circle = plt.Circle((x, y), config.robot_radius, color="pink")
...
def main(config, configTwo):
    # initial state [x(m), y(m), yaw(rad), v(m/s), omega(rad/s)]
    x1 = np.array([0.0, 0.0, math.pi / 2.0, 0.0, 0.0])
    ...
    x2 = np.array([0.0, 20.0, -math.pi / 2.0, 0.0, 0.0])
    # goal position [x(m), y(m)]
    goal2 = configTwo.target
    # input [forward speed, yaw_rate]
    trajectory2 = np.array(x2)
    ob2 = configTwo.ob
    dwa2 = DWA(configTwo)
    ...
    while True:
        ...
        u2, predicted_trajectory2 = dwa2.dwa_control(x2, goal2, ob2)
        x2 = KinematicModel(x2, u2, configTwo.dt)  # simulate robot
        trajectory2 = np.vstack((trajectory2, x2))  # store state history

        u1, predicted_trajectory1 = dwa1.dwa_control(x1, goal1, ob1)
        x1 = KinematicModel(x1, u1, config.dt)  # simulate robot
        trajectory1 = np.vstack((trajectory1, x1))  # store state history
        ...
        plt.text(start_A_X, start_A_Y, "Start_A")
        plt.text(start_B_X, start_B_Y, "Start_B")
        plt.text(goal1[0], goal1[1], "Target A")
        plt.text(goal2[0], goal2[1], "Target B")

        plt.plot(predicted_trajectory1[:, 0], predicted_trajectory1[:, 1], "-g")
        plt.plot(x1[0], x1[1], "xr")
        plt.text(x1[0], x1[1], "A", fontsize = 20)
        ...
        plt.text(x2[0], x2[1], "B", fontsize = 20)
        plt.plot(goal2[0], goal2[1], "xb")
        plot_robot(x2[0], x2[1], x2[2], configTwo)
        ...
        if dist_to_goal1 <= config.robot_radius and dist_to_goal2 <= config.robot_radius:
            print("Goal!!")
            break
